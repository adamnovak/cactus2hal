                              ...
                              string& outSequence)
{
  ...
  {
    // For each FASTA line that isn't a comment or header
    getline(_faFile, buffer);
    
    // Copy the line (without trailing newline) to the output sequence.
    outSequence += buffer;
    
    // Advance to the next non-empty, non-comment line, which may be more
    // sequence or a new header.
    skip();
  }
